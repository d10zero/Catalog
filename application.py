...
def latestItems():
    ...
    if 'username' not in login_session or login_session['username'] == None:
        return render_template('publicLatestItem.html', categories = categories, items = items, header = "Latest")
    else:
        print login_session['username']
        return render_template('latestItem.html', categories = categories, items = items, header = "Latest")
...
def selectedCategory(category): 
    ...
    items = session.query(Item).filter_by(category_id= selectedCatagory.id).all()
    if 'username' not in login_session or login_session['username'] == None:
        return render_template('publicLatestItem.html', categories=categories, items=items, header=category)
    else:
        return render_template('latestItem.html', categories=categories, items=items, header=category)
...
def selectedItem(category, item):
    ...
    print 'selected item: ' + str(selectedItem.user.username)
    if 'username' not in login_session or login_session['username'] == None:
        user = False
        ...
        user = False
    return render_template('itemDescription.html', header = itemName, description = selectedItem.description, user = user)
...
def addItem():
    if 'username' not in login_session or login_session['username'] == None:
        ...
        if existingItem > 0:
            ...
            formCategory = session.query(Category).filter_by(name=formCategoryName).one()
            formUsername = login_session['username']
            formUser = session.query(User).filter_by(username=formUsername).one()
            item = Item(name=formName, description=formDescription, category=formCategory, user=formUser)
            session.add(item) 
            session.commit()
            return redirect(url_for('selectedCategory', category=formCategoryName))
    else:
        categories = session.query(Category).all()
...
def editItem(item):
    ...
    if 'username' not in login_session or login_session['username'] == None:
        ...
        if request.method == 'POST':
            ...
            selectedItem.category = session.query(Category).filter_by(name = request.form['category']).one()
            session.add(selectedItem) 
            session.commit()
            return redirect(url_for('selectedItem', category=selectedItem.category.name, item=selectedItem.name))
        else:
            print 'item selected in edit: ' + str(selectedItem.name)
            return render_template('editItem.html', item=selectedItem, categories = categories)
...
def login(provider):
    if provider == 'google':
        ...
        try:
            ...
            response = make_response(json.dumps('Failed to upgrade the authorizate code.'), 401)
